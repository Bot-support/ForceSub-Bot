...
async def _(event):
    ...
    if event.user_joined or event.user_added:
        ...
        pp = await Bot.get_participants(chat)
        count = len(pp)
        ...
        if x is True:
            ...
            await Bot.edit_permissions(event.chat.id, user.id, until_date=None, send_messages=False)
...
async def mute_on_msg(event):
    ...
    temp = await Bot(GetFullUserRequest(event.sender_id))
    if x is False:
        if temp.user.bot:
            return
        nm = temp.user.first_name
        try:
            await Bot.edit_permissions(event.chat.id, event.sender_id, until_date=None, send_messages=False)
        except Exception as e:
            print(str(e))
            return
...
@Bot.on(events.callbackquery.CallbackQuery(data=re.compile(b"unmute_(.*)")))
async def _(event):
    uid = int(event.data_match.group(1).decode("UTF-8"))
    if uid == event.sender_id:
        x = await get_user_join(uid)
        nm = (await Bot(GetFullUserRequest(uid))).user.first_name
        if x is False:
            await event.answer(f"You haven't joined @{channel} yet!", cache_time=0, alert=True)
        elif x is True:
            try:
                await Bot.edit_permissions(event.chat.id, uid, until_date=None, send_messages=True)
            except Exception as e:
                print(str(e))
                return
...
async def strt(event):
    await event.reply(f"Hi. I'm a force subscribe bot made specially for @{channel}!\n\nCheckout @DeCodeSupport :)", buttons=[Button.url("Channel", url=f"https://t.me/{channel}"), Button.url("Repository", url="https://github.com/TeamDeeCode/FORCESUB-BOT")])

    
print("ForceSub Bot has started.\nDo visit @DeCodeSupport!")
